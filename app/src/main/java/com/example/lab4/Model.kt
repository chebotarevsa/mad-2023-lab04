...
object Model {
    ...
    fun removeCard(id: Int) { //удаление по id
        _cards.removeIf {
            it.id == id
        }
    }

    fun addCard(card: Card) { //добавления нового элемента
        _cards.add(card)
    }
    ...
    fun updateCardList(position: Int, card: Card) { //Удаление по id карточки и замена
